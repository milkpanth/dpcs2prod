...
const { TEMP_OUTPUT, PDF_PREFIX, PREVIEW_PREFIX } = require("../constants");
...
const convertPDF2 = async (fileName, isPreview) => {
    const pdfOptions = new api.PdfExportOptions()
    //pdfOptions.drawSlidesFrame = false
    //pdfOptions.jpegQuality = 100
    //pdfOptions.sufficientResolution = 100
    pdfOptions.format = "pdf"
    pdfOptions.textCompression = api.PdfExportOptions.TextCompressionEnum.None
    //pdfOptions.compliance = api.PdfExportOptions.ComplianceEnum.PdfUa
    pdfOptions.notesPosition = api.PdfExportOptions.NotesPositionEnum.None
    console.log(`${new Date().toISOString()} Converting PDF via ASPOSE...`)
    const res = await slidesApi.convert(createReadStream(`${TEMP_OUTPUT}/${fileName}`), "pdf", null, null, null, null, pdfOptions)
    //const res = await slidesApi.downloadPresentation(fileName, api.ExportFormat.Pdf)
    const pdfFileName = `${fileName}${(isPreview) ? PREVIEW_PREFIX : PDF_PREFIX}.pdf`
    console.log(`${new Date().toISOString()} Writing converted PDF file to local...`)
    await fs.writeFile(`${TEMP_OUTPUT}/${pdfFileName}`, res.body);
    //await uploadBlob(res.body, `${fileName}${(isPreview) ? PREVIEW_PREFIX : PDF_PREFIX}.pdf`, "application/pdf")
...
module.exports = { convertPDF, mergePPTX }