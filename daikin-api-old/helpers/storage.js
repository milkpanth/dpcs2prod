...
const { TEST, MEDIA } = require("../constants/container")

const blobServiceClient = BlobServiceClient.fromConnectionString(process.env.AZURE_STORAGE_CONNECTION_STRING);


module.exports = {
    async uploadBlob(fileBuff, blobName, fileType) {
        const containerClient = blobServiceClient.getContainerClient(TEST);
        
        const blockBlobClient = containerClient.getBlockBlobClient(blobName);
        const uploadResponse = await blockBlobClient.uploadData(fileBuff, { blobHTTPHeaders: { blobContentType: fileType } });
        console.log(`Uploaded Blob: ${blobName}`)

        return uploadResponse
    },
    async uploadFile(filePath, blobName, fileType) {
        const containerClient = blobServiceClient.getContainerClient(TEST);
        
        const blockBlobClient = containerClient.getBlockBlobClient(blobName);
        const uploadResponse = await blockBlobClient.uploadFile(filePath, { blobHTTPHeaders: { blobContentType: fileType } });
    ...
    async deleteBlob(blobName) {
        ...
        console.log(`${new Date().toISOString()} - Recieved Delete Blob: ${blobName}`)
    ...
    async downloadToTemp(blobName, containerName = TEST){
        const fileName = `${TEMP_OUTPUT}/${blobName}`
        const isInLocal = await fs.stat(fileName).then(() => true).catch(() => false)
        if(isInLocal){
            return fileName
    ...
    STORAGE_PATH: `https://${blobServiceClient.accountName}.blob.core.windows.net/${TEST}/`,
    getFormmatedName,
...
function getFormmatedName(prefixName, blobName, languageCode){
    if(languageCode){
        return `${prefixName}_${formatDate(new Date())}_${languageCode}.${getExtensionFromString(blobName)}`
    }
    return `${prefixName}_${formatDate(new Date())}.${getExtensionFromString(blobName)}`
}
