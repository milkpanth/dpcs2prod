...
const { uploadFile, downloadToTemp, deleteBlob, getFormmatedNameProposal, STORAGE_PATH, formatDate } = require("../helpers/storage")
...
const { PENDING, WORKING, SUCCESS, ERROR } = require("../constants/pendingproposal")
...
module.exports = {
    async dataTable(req, res) {
		...
		try {
            ...
            const atLeastTime = new Date(new Date().setDate(new Date().getDate() - DAY_AT_LEASET));
			...
			if (req.body.search.value !== "") {
				const resultRecord = await Proposal.findAndCountAll({
                    attributes: [
                        ...
                        [sequelize.fn("CONCAT", "proposal/download/pdf/", sequelize.col("Proposal.ProposalID")), "PDFFullPath"],
                        [sequelize.fn("CONCAT", "proposal/download/pptx/", sequelize.col("Proposal.ProposalID")), "PPTXFullPath"]
                    ],
					where: {
						[Op.or]: [{
							ProposalID: {
			...
			if (req.body.search.value !== "") {
				const resultRecord = await Proposal.findAndCountAll({
                    attributes: [
                        ...
                        "ExpireDate",
                        "DeletedDate",
                        [sequelize.fn("CONCAT", STORAGE_PATH, sequelize.col("PDFFile")), "PDFPreview"],
                        [sequelize.fn("CONCAT", "proposal/download/pdf/", sequelize.col("Proposal.ProposalID")), "PDFFullPath"],
                        [sequelize.fn("CONCAT", "proposal/download/pptx/", sequelize.col("Proposal.ProposalID")), "PPTXFullPath"]
                    ],
					where: {
    ...
    async readEquipment(req, res) {
		try {
            ...
            const { sectionArray, subSectionArray, dataArray } = await getSection(array, user, lang)
            res.json({
                sectionArray,
                subSectionArray,
                dataArray
            ...
            switch(firstLetter){
                case "P":
                    await Proposal.create({
                        ...
                        "CompanyProfileLanguage": pendingProposal.ProposalData.CompanyProfileLanguage,
                        "CreatedBy": oid
                    })
                    await ProjectSectionList.create({
                        ...
                        "SubSectionArrayText": pendingProposal.SectionList.SubSectionArrayText,
                    })
    ...
    async generateProject(req, res) {
		try {
            ...
            q.push(() => doCreateProjectAspose(project, profileLangauage, section, subsection, data, projectID, user, pendingProposal))
    ...
    async generateGeneric(req, res) {
        try{
            ...
            q.push(() => doCreateGenericAspose(project, profileLangauage, generic, projectID, user, pendingProposal))
    ...
    async delete(req, res) {
		try {
            ...
            createLog(req, oid, PROPOSAL_FN, `${ptype === "PROJECT" ?  DELETE_PROJECT : DELETE_GENERIC} ${pid}`)
    ...
    async downloadPowerPoint(req, res){
        try {
			...
			res.set("Content-Disposition", `attachment; filename=${getFormmatedNameProposal(proposal.ProjectName, proposal.PPTXFile)}`)
    ...
    async downloadPDF(req, res){
        try {
			...
			res.set("Content-Disposition", `attachment; filename=${getFormmatedNameProposal(proposal.ProjectName, proposal.PDFFile)}`)
...
// For AsposeCloud
async function doCreateProjectAspose(projectProperty, profileLangauage, section, subsection, data, projectID, user, pendingProposal){
    try{
        const fileList = []
        ...
        for(const current of sections){
            if(current.SectionID == COVER_SECTION_ID){
                ...
                const coverFilePath = await genCoverPage(projectProperty, projectID, company)
            ...
            if(current.SectionID == EQUIP_SECTION_ID){
                ...
                const allSection = await CompanySection.findAll({
                    where: {
                        SectionID: { 
                            [Op.notIn]: [COVER_SECTION_ID,PROFILE_SECTION_ID,EQUIP_SECTION_ID, END_SECTION_ID]
                        }, 
                        CompanyCode: user.CompanyCode
                    },
                    order: [["CompanyOrder", "ASC"]],
                ...
                const company = await CompanyProfile.findByPk(user.CompanyCode)
                const equipFilePath = await genEquipmentList(allUnique, company, allSection)
        ...
        uploadFile(`${TEMP_OUTPUT}/${pptxFile}`, pptxFile, "application/vnd.openxmlformats-officedocument.presentationml.presentation")
        const pdfFile = await convertPDF(pptxFile)
        uploadFile(`${TEMP_OUTPUT}/${pdfFile}`, pdfFile, "application/pdf")

        pendingProposal.ProposalData = { ...pendingProposal.ProposalData, PPTXFile: pptxFile, PDFFile: pdfFile }
...
async function doCreateGenericAspose(projectProperty, profileLangauage, generic, projectID, user, pendingProposal){
    try{
        const fileList = []
...
async function doCreateGeneric(projectProperty, generic, projectID){
    ...
    pres.addSlide("Cover", 1, (slide) => applyCoverModify(slide, projectProperty, projectID))

    const filePath = await downloadToTemp(generic.Path)
    pres.load(filePath)
    const matchedObject = (await pres.setCreationIds()).find(x => x.name == filePath)
    matchedObject.slides.forEach(x => { pres.addSlide(filePath, x.id) })
...
async function genCoverPage(projectProperty, projectID, companyProperty){
    ...
    .addImage({ path: "./templates/Cover_BG_R.png", x: 0.65, y: 0, w: 12.68, h: 7.5 })
    .addImage({ path: "./templates/Cover_BG_L.png", x: 0, y: 0, w: 3.91, h: 7.5 })
    .addImage({ path: "./templates/Date_Gradient.png", x: 1.31, y: 1.77, w: 1.92, h: 0.3 })
    .addImage({ path: "./templates/DKLogo.png", x: 0.23, y: 0.25, w: 1.96, h: 0.56 })
    ...
    slide.addText(projectProperty.name, { 
        w: 8.05,
        h: 2.52,
        x: 1.16,
        y: 2.25,
...
async function genEquipmentList(data, company, sections){
    ...
    data.sort(
        (a, b) =>{                              // Pass a function to the sort that takes 2 elements to compare
            if(a.Class == b.Class){                    // If the elements both have the same `type`,
    ...
    for(let i = 0; i < data.length; i++){
        ...
        if(i % 10 == 0){
            slide = pres.addSlide()
            .addImage({ path: "./templates/EquipmentSummary_Gradient.png", x: 0, y: 0.95, w: 0.96, h: 0.3 })
            ...
            .addImage({ path: "./templates/EquipmentSummary_Footer.png", x: 0, y: 7.01, w: 13.33, h: 0.51 })
        ...
        .addText(`(${each.Quantity})${each.Series}${each.Description ? " " + each.Description: ""}`, { 
            w: 2.69,
            h: 0.44,
            x: start_x + 3.13 + nx,
            y: start_y + ny,
        ...
        if(seriesData){
            if(seriesData.ImagePath){
                const filepath = await downloadToTemp(seriesData.ImagePath, MEDIA).catch(err => {
...
async function getSection(array, user, lang){
    ...
    for(const data of array){
        ...
        const modelLang = new Set()
        modelFiles.forEach(f => {
            if([CU_S_GENERAL, FCU_S_GENERAL, CC_S_GENERAL].includes(f.SubSectionID)){
                modelLang.add(f.LangID)
            }
        })
        if(modelLang.size == 0){ continue }
        ...
        const seriesLang = new Set()
        seriesFiles.forEach(f => {
            seriesLang.add(f.LangID)
        })
        ...
        dataArray.push({
            ...
            "ModelLanguage": Array.from(modelLang),
            "SeriesLanguage": Array.from(seriesLang),
            "SelectedLanguage": data.LangID || null
        })
    }

	return { 
        sectionArray: Array.from(sectionArray),
