...
const { mergePPTX } = require("../helpers/merger");
const {
  CREATE_PROJECT,
  CREATE_GENERIC,
  PROPOSAL_FN,
...
module.exports = {
  ...
  async getSummery(req, res){
    try{
      const user = await getUser(req);
      const { UserMemberID, CompanyCode } = user
      const date = new Date(), y = date.getFullYear(), m = date.getMonth();
      const firstDay = new Date(y, m, 1);
      const lastDay = new Date(y, m + 1, 0);

      const newProposal = await Proposal.count({
        where: {
          [Op.and]: [{
      ...
      const editProposal = await Proposal.count({
        where: {
          [Op.and]: [{
            UpdatedBy: UserMemberID,
            UpdatedDate: {
              [Op.between]: [firstDay, lastDay],
            },
          }]
      ...
      const companyRank = await Proposal.findAll({
        attributes: [
          'CreatedBy',
          [sequelize.fn('COUNT', sequelize.col('*')), 'TotalCount'],
          [sequelize.literal('RANK () OVER (ORDER BY COUNT(*) DESC)'), 'CompanyRank']
        ],
        include: [
          {
            model: UserMember,
  ...
  async readEquipment(req, res) {
    try {
      ...
      const matchedArray = await getSection(
        array,
        lang
      );
  ...
  try {
    ...
    for (const current of sections) {
      ...
      if (current.SectionID == END_SECTION_ID) {
        console.log("GenEnd");
        console.log("Copy EndPage from template")
        const endTemplateFile = "End.pptx"
        const newEndTemplate = "End_" + Date.now() + ".pptx"
        await fs.copyFile(`${TEMPLATE_DIR}/${endTemplateFile}`, `${TEMP_OUTPUT}/${newEndTemplate}`)
...
async function appendSlideAspose(list, file, download = true) {
  ...
  if (download) {
    await downloadToTemp(file.Path, "test");
  }
  list.push({
    model: file.ModelID,
...
async function getSection(array, lang) {
  const matchModels = await Model.findAll({
    include: [
      {
        attributes: ["CompanyCode","LangID"],
        model: ModelFile,
        where: {
          LangID: lang
        },   
    ...
    where: {
      ModelID: array.map(m => m.Model)
    }
  });
  const matchedArray = matchModels.map(m => {
    const rm = array.find(x => x.Model == m.ModelID)
    return {
      Model: m.ModelID,
      Series: m.SeriesID,
      Type: convertClass(m.ClassID),
      Qty: rm.Quantity,
      Desc: rm.Description,
      Files: m.ModelFiles
    }
