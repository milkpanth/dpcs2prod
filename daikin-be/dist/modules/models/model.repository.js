...
let ModelRepository = class ModelRepository {
    constructor(useModelRepo, liveModelRepo, logger) {
        this.useModelRepo = useModelRepo;
        this.liveModelRepo = liveModelRepo;
        this.logger = logger;
        this.logger.setContext("ModelRepository");
    }
    async findOneWithWhereConditions(whereConditions, databaseType, relations) {
        ...
        if (databaseType === database_type_enum_1.DataBaseTypeEnum.USE) {
            repo = this.useModelRepo;
        }
        else {
            repo = this.liveModelRepo;
        }
    ...
    async insertOrUpsert(Model, databaseType) {
...
ModelRepository = __decorate([
    ...
    __param(1, (0, typeorm_1.InjectRepository)(model_entity_1.Model, db_1.LIVE_DB_NAME)),
